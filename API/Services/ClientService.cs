...
namespace API.Services
{
    public class ClientService : IClientService
    {
        ...
        public IEnumerable<Client> GetClientsByEmail(string email) => _clients.Where(client => client.Email.ToLower().Equals(email.ToLower())).OrderBy(client => client.Id);
        public IEnumerable<Client> GetClientsByName(string name) => _clients.Where(client => client.Name.ToLower().Equals(name.ToLower())).OrderBy(client => client.Id);
